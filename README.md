...
```powershell
git clone --branch analysis https://github.com/BAITC-Hacks/hack-63569893-ml-empire.git voice-router
cd voice-router
```
...
Скрипт последовательно отправляет все 104 реплики маршрутизатору. Для каждой используется пустой диалоговый контекст, поэтому здесь измеряется распознавание отдельного обращения. Прогресс выводится каждые 10 реплик и в конце. Затем печатаются медиана времени маршрутизации и метрики из эталонного оценщика.

Для сравнения тем же скриптом предусмотрена вторая модель:

```powershell
uv run --project backend --env-file .env python backend/scripts/evaluate_router.py --model gpt-6-luna
```

Это реальные платные обращения к API. Команда не использует микрофон и не запускает STT или TTS. Отсутствие ключа завершает скрипт с кодом `2`. Если маршрутизатор вернул ошибку, прогон останавливается с указанием реплики и кодом `1`, чтобы не выдавать технические сбои за обычные предсказания.

Текущая версия скрипта создаёт `predictions.json` во временной папке для расчёта метрик и удаляет её после завершения. Постоянный файл результатов и аргумент `--output` у этой команды не предусмотрены. Результат штатного запуска — отчёт в терминале.
...
Для CLI-оценщика используйте базовый объект из раздела 10.3. Расширенный объект с полями `predictions`, `latency_ms` и `latency_stage` предназначен для импорта в интерфейсе.
...
Описание серверных проверок находится в [backend/README.md](backend/README.md), отчёты и сценарии клиентской проверки — в [frontend-verification.md](frontend/docs/frontend-verification.md). Они дополняют этот README подробностями отдельных выполненных проверок.
...
├── backend/
│   ├── README.md                 Серверные команды и детали реализации
...
├── frontend/
│   ├── README.md                 Краткое руководство по frontend
...
| --- | --- |
| [Backend README](backend/README.md) | Нужны серверные команды, внутренние решения и подробности проверки backend |
| [Frontend README](frontend/README.md) | Нужны команды запуска интерфейса и быстрые ссылки |
