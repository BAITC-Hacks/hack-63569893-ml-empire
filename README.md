...
Этот README объясняет проект от пользовательской задачи до устройства кода. Для первого знакомства можно запустить локальный тестовый режим. Для разговора с настоящими моделями предусмотрен отдельный путь запуска с сервером и API-ключом.
...
**Как познакомиться.** [Режим без ключа](#fixture) показывает интерфейс на подготовленных ответах. [Полный запуск](#live) подключает настоящие модели. [Сценарий проверки](#walkthrough) объясняет, какие фразы отправить и что посмотреть в результате.
...
10. [Лабораторные инструменты интерфейса](#labs)
...
| Клиентская разработка | Реализованы голосовой и текстовый ввод, история, подтверждения, воспроизведение, панель супервизора и лаборатории | Пользователь взаимодействует с приложением, а команда видит ход обработки и может разбирать результаты |
...
Отдельные разделы помогают работать с каталогом, размеченными диалогами, результатами оценки и аудиоустройствами. Их назначение и порядок использования описаны ниже.
...
| --- | --- | --- |
| Интерфейс | React 19 и TypeScript | Экран разговора, состояния, формы, панели и лаборатории |
...
`/audio` воспроизводит тон, а не синтезированную речь. `/emotion` передаёт искусственно подготовленные поля. Проверять качество STT, TTS, маршрутизации или определения эмоций по этим примерам нельзя. В разговоре этого режима используйте только текст: отправка аудио через микрофон вызовет ошибку тестового сервера. Для проверки самого устройства откройте раздел **«Микрофон и звук»**; для разговора с моделями используйте следующий раздел README.

В интерфейсе также есть кнопка **«Посмотреть пример»**. Она открывает заранее подготовленный фрагмент разговора и трассировку. Ввод в этом представлении отключён. Это быстрый просмотр экрана, который не создаёт настоящую серверную сессию.
...
Для автоматического режима раскройте **«Настройки микрофона» → «Режим записи» → «Завершать после тишины»**. Нажмите кнопку микрофона один раз: запись начнётся и закончится после паузы. Можно завершить её вручную. В настройках доступны:
...
<a id="labs"></a>
## 10. Лабораторные инструменты интерфейса

В приложении есть пять разделов: **«Разговор»**, **«Сценарии»**, **«Диалоги датасета»**, **«Оценка и сравнение»**, **«Микрофон и звук»**. Основной разговор уже описан выше. Остальные разделы помогают изучать данные и проверять отдельные части системы.
...
### 10.2. «Диалоги датасета»: эталон и реальный прогон

В разделе доступны 10 размеченных разговоров. Они показывают, как отдельные реплики связаны в обслуживание: клиент задаёт вопрос, уточняет сведения, подтверждает действие или переходит к следующей теме.

В режиме просмотра эталона можно двигаться по одной реплике, запустить показ с интервалом около 1,8 секунды, поставить его на паузу, вернуться к началу или раскрыть весь диалог. На экране отображаются готовые клиентские и ответные реплики из набора данных. В этом режиме запросы к моделям не выполняются.

Для прогона через API запускается новая серверная сессия. Такой запуск очищает предыдущую историю текущего разговора. Каждую клиентскую реплику пользователь отправляет отдельно и сравнивает ожидаемые сценарии с фактическими маршрутами, ответом и трассировкой.

В API передаётся текст клиента. Эталонные идентификаторы сценариев и ожидаемые слоты не подмешиваются в запрос маршрутизатору. Шаг с подтверждением действия требует отдельного согласия пользователя в интерфейсе. Автоматического подтверждения, продолжения после ошибки или повторной отправки нет.

Результаты можно сохранить в `<dialog_id>-results.json`. Это отчёт о последовательном диалоге. Его структура отличается от файла предсказаний для 104 отдельных реплик, который используется в следующей лаборатории.

### 10.3. «Оценка и сравнение»: качество выбора сценариев

Эта лаборатория получает готовые предсказания и сравнивает их с разметкой `dev_utterances.json`. Она не запускает LLM сама. Два поля сравнения Full и Fast обозначают два импортированных набора результатов. Названия не переключают модель или режим обработки на сервере.
...
Можно вставить JSON в поле или загрузить файл до 1 МиБ. Проверяются известные сценарии, отсутствие повторов внутри массива и допустимость задержек: конечное число от 0 до 600 000 мс. Если версия датасета указана, ожидается `1.0`. Неизвестные идентификаторы реплик учитываются как лишние и не участвуют в оценке.
...
### 10.4. «Микрофон и звук»: локальная диагностика

Раздел помогает проверить устройство до разговора с сервером. Он запрашивает разрешение, показывает уровень сигнала, RMS, пик и длительность, записывает короткий фрагмент, воспроизводит его и проигрывает тестовый тон.

Запись ограничена 15 секундами. Автоматическое завершение по тишине включено по умолчанию; исходные настройки — 1 500 мс и порог 0,025. Ожидание разрешения ограничено примерно 20 секундами. Тестовый тон длится около двух секунд.

Диагностическая запись остаётся в памяти браузера и удаляется при выходе из раздела. Она не отправляется в STT или другие модели. Отдельной кнопки скачивания записи нет. Показатель задержки измеряется по часам браузера при запуске локального проигрывателя. Он не измеряет физический момент звучания динамика или время обработки запроса моделями.

<a id="data"></a>
## 11. Данные и интеграции

В каталоге [datas/](datas/) находится синтетический набор страхового контакт-центра. Он задаёт предметную область, допустимые действия, факты для ответов и примеры для оценки.

| Файл | Содержимое | Кто использует |
| --- | --- | --- |
| [scenarios.json](datas/scenarios.json) | 40 предметных сценариев `SC01`–`SC40` и 3 системных сценария | LLM-маршрутизатор, граф и редактор каталога |
| [slots.json](datas/slots.json) | 43 поля с типами, ограничениями и вопросами для сбора | Проверка и заполнение параметров |
| [actions.json](datas/actions.json) | 31 действие, включая 9 с обязательным подтверждением; очереди операторов и коды ошибок | Обработчики и правила сценариев |
| [knowledge_base.json](datas/knowledge_base.json) | 6 продуктов, 8 офисов, 3 площадки осмотра, 6 клиник и другие сведения компании | Ответы на справочные вопросы |
| [mock_backend.json](datas/mock_backend.json) | 11 клиентов, 11 полисов, 4 страховых случая и 2 платежа | Демонстрационные операции backend |
| [dialogs_sample.json](datas/dialogs_sample.json) | 10 диалогов: 40 клиентских и 40 ответных реплик | Проигрыватель диалогов и проверки графа |
| [dev_utterances.json](datas/dev_utterances.json) | 104 размеченные отдельные реплики | Оценка маршрутизатора и сравнение предсказаний |
| [evaluate.py](datas/evaluate.py) | Скрипт расчёта метрик | Офлайн-оценка готового JSON с предсказаниями |

В каталог сценариев включены 160 примеров на русском и 120 на казахском. Системные сценарии — `SYS_OUT_OF_SCOPE`, `SYS_UNCLEAR` и `SYS_GOODBYE`: обращение вне предметной области, неясный запрос и завершение разговора.

В наборе отдельных реплик 52 русских, 45 казахских и 7 смешанных. По типу намерения это 84 одиночных запроса, 13 запросов с несколькими намерениями, 4 обращения вне области и 3 неясных обращения. Среди 40 предметных сценариев 30 имеют обычный приоритет, 7 — высокий и 3 — срочный.

Данные привязаны к контрольной дате **1 октября 2026 года**. Относительные даты в демонстрационных действиях рассчитываются относительно этого состояния набора. Например, «вчера» соответствует 30 сентября 2026 года. Это делает поведение воспроизводимым при работе с фиксированными полисами и записями.

Аудиозаписей и размеченного корпуса произношения в этих файлах нет. Набор отдельных реплик позволяет оценивать выбор сценария по тексту. Он сам по себе не измеряет точность распознавания речи, естественность синтеза или качество работы с реальными телефонными шумами.

Подробное описание исходного набора доступно на [русском](datas/README.ru.md), [казахском](datas/README.kz.md) и [английском](datas/README.md).

### Подключённые внешние сервисы

| Интеграция | Какие данные получает | Что возвращает |
| --- | --- | --- |
| OpenAI Responses API через LangChain | Текст обращения, недавний контекст, описание сценариев и допустимых полей | Структурированное решение маршрутизатора |
| OpenAI Realtime transcription | Аудиофрагменты текущей голосовой реплики | Промежуточную и итоговую расшифровку |
| OpenAI speech API | Подготовленный сервером текст ответа | Аудио для воспроизведения в браузере |

Эти вызовы выполняет backend. Браузер получает результат через API проекта и не хранит ключ провайдера. Страховые сведения берутся из локальных JSON и копий mock-данных в памяти; внешняя CRM или база страховой компании в текущую цепочку не включена.
...
| Импортированные оценки и результаты прогона диалога | Состояние интерфейса | Сохраняются при переходе между соответствующими разделами, очищаются при перезагрузке |
| Диагностическая запись микрофона | Память компонента диагностики | Удаляется при выходе из раздела |
...
Для проверки настоящей обработки выполните [полный запуск](#live) и откройте `http://127.0.0.1:5173`. Фразы ниже взяты из примеров каталога. Ожидаемый маршрут описывает целевое поведение сценария; фактическое решение модели видно в трассировке.
...
| `npm.cmd --prefix frontend test` | Протокол клиента, состояния сессии, аудио, модели лабораторий, супервизора и тестового API |
...
`--output` сохраняет расширенный JSON в указанный файл; родительская папка должна существовать. Отчёт содержит модель, effort, параллельность, `predictions`, `latency_ms`, `latency_stage="router"`, сводки и категории ошибок. Исходные тексты реплик не сохраняются. Такой файл можно импортировать в лабораторию **«Оценка и сравнение»**.
...
Для CLI-оценщика используйте базовый объект из раздела 10.3. Если предсказания взяты из расширенного отчёта, передайте только содержимое его поля `predictions`. Полный объект с полями `predictions`, `latency_ms` и `latency_stage` предназначен для импорта в интерфейсе.
...
Для многошагового поведения откройте **«Диалоги датасета»**, выберите пример и отправляйте клиентские реплики в реальный API по одной. На каждом шаге сопоставляйте выбранный маршрут, вопрос робота, собранные параметры и выполненное действие. Подтверждение операции проверяйте отдельно от правильности классификации её названия.

Для голосового пути сначала проверьте устройство в **«Микрофон и звук»**, затем проведите настоящий разговор. Разберите отдельно расшифровку, сценарий, текст ответа и его звучание. Если текст распознан неверно, последующая ошибка маршрута может быть следствием STT; если текст верен, искать причину следует уже в маршрутизации или диалоговом состоянии.
...
### 15.6. Зафиксированные результаты маршрутизации и голоса

В [отчёте от 23 сентября 2026 года](docs/benchmarks/2026-09-23-voice-latency.md) сохранены результаты обработки 104 синтетических обращений через настоящего провайдера:

| Реализация | Параллельность | Главный сценарий | Полный набор намерений | p50 | p95 |
| --- | ---: | ---: | ---: | ---: | ---: |
| Исходный Sol, `low` | 3 | 104/104 | 103/104 | 2 983 мс | 4 234 мс |
| Компактный Luna, `none` | 3 | 104/104 | 102/104 | 1 847 мс | 2 701 мс |
| Компактный Sol, `none` | 2 | 104/104 | 104/104 | 2 183 мс | 3 103 мс |

В этих трёх прогонах не было ошибок провайдера и валидации. Исходный Sol пропустил второе намерение в U083; Luna — в U086 и U090. Для текущей конфигурации выбран компактный Sol. Совпадение 104/104 относится к этому набору и не гарантирует такую же точность на новых разговорах.

Исходный маршрутизатор загружался из коммита `d141057`. Прогоны исходного Sol и Luna частично выполнялись одновременно, а компактный Sol использовал другую параллельность. Нагрузка сети и провайдера не контролировалась, поэтому таблица не доказывает фиксированный процент ускорения. Проверить отдельные предсказания и замеры можно в JSON: [исходный Sol](docs/benchmarks/2026-09-23-router-baseline.json), [компактный Luna](docs/benchmarks/2026-09-23-router-compact-luna.json), [компактный Sol](docs/benchmarks/2026-09-23-router-compact-sol.json).

После переноса сборки графа в запуск приложения отдельная первая текстовая реплика об офисе получила SC33 без ошибок. В отчёте зафиксированы 2 794 мс для маршрутизации, 904 мс до первого TTS-фрагмента и 3 732 мс до первой отправки PCM сервером. Время вне маршрутизации и старта TTS составило около 34 мс; до изменения в отдельном первом обращении оно составляло около 1 305 мс. Это наблюдения отдельных запусков.

В отдельной голосовой проверке искусственно созданная реплика прошла через прокси frontend, настоящий STT, граф и TTS и получила SC33. От проверенного итогового текста STT до первой отправки PCM прошло **3 131 мс**; завершение STT заняло **3 957 мс**. Клиент наблюдал **8 303 мс** от `turn.commit` до первого полученного PCM, включая ожидание обработки и транспорта. Реальный микрофон в этом прогоне не записывался. Эти числа не являются p95 голосовых разговоров или измерением от конца человеческой речи до звучания динамика.

Все замеры маршрутизации в трёх наборах превысили 500 мс. Цели 500 мс для маршрутизатора и 1 500 мс от итогового STT до первой отправки аудио этими результатами не подтверждены. Приведённые цифры взяты из сохранённого отчёта, а не из повторного запуска при подготовке README.
...
| Микрофон не найден или отключился | Выберите доступное устройство, подключите его заново и проверьте в разделе диагностики |
...
| Текст ответа есть, звука нет | Проверьте mute, громкость, локальный тестовый тон и наличие `tts_unavailable` |
| В fixture микрофон вызывает ошибку | Разговор с тестовым API принимает текст; локальная проверка устройства находится в диагностике |
...
│   │   ├── dataset-model.ts      Работа с размеченными диалогами
│   │   ├── evaluation-model.ts   Импорт и расчёт метрик
│   │   ├── diagnostics-model.ts  Локальная аудиодиагностика
│   │   ├── affect-model.ts       Отображение дополнительных метаданных
│   │   └── components/           Панели и лаборатории
...
| [Лаборатории frontend](frontend/docs/frontend-labs.md) | Нужны детали редактора, диалогов, оценки и аудиодиагностики |
...
Публичный URL демоверсии в репозитории не указан. Подтверждённый способ знакомства с проектом — локальный запуск по инструкциям этого README:
...
Эти адреса доступны на компьютере, где запущен проект, и не являются публичной deployed-версией.
