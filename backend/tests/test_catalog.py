...
def test_catalog_loads_valid_dataset_and_resolves_references():
    ...
    assert len(catalog.scenarios) == len({spec.scenario_id for spec in catalog.scenarios.values()})
    assert len(catalog.actions) == len({spec.name for spec in catalog.actions.values()})
